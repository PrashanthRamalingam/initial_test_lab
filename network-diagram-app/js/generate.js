...
function inferTypeFromHostname(name) {
  // Match hints against the name's alphabetic tokens (split on digits and
  // separators) so "DMZSW" -> switch and "NGA-SC01" -> switch, while a
  // hint never fires from the middle of an unrelated word.
  const tokens = String(name).toLowerCase().split(/[^a-z]+/).filter(Boolean);
  const hints = [...HOSTNAME_TYPE_HINTS].sort((a, b) => b[0].length - a[0].length);
  for (const [hint, type] of hints) {
    for (const token of tokens) {
      if (token === hint || token.startsWith(hint) || token.endsWith(hint)) return type;
    }
  }
  return 'server';
}
...
function layoutLayers(layers) {
  const CROSS_GAP = 170, ROW_GAP = 150, CENTER = 420;
  // Wrap wide layers, but let big imported graphs spread out instead of
  // folding into a dense grid.
  const widest = layers.reduce((m, l) => Math.max(m, l.length), 0);
  const PER_ROW = Math.max(6, Math.min(12, Math.ceil(Math.sqrt(widest * 2))));
...
function layoutGraph(nodes, edges) {
  ...
  // A cycle (a BGP session pointing back at the cloud, a ring) has no
  // valid layering: depth-first search finds the edges that close each
  // loop and leaves them out of the ranking. They are still drawn.
  const forward = edges.filter(e => !isPeer(e));
  const adjacency = {};
  for (const n of nodes) adjacency[n.id] = [];
  for (const e of forward) if (adjacency[e.from]) adjacency[e.from].push(e);
  const visitState = {};
  const backEdges = new Set();
  const walk = (id) => {
    visitState[id] = 1;
    for (const e of adjacency[id] || []) {
      if (visitState[e.to] === 1) backEdges.add(e);
      else if (!visitState[e.to]) walk(e.to);
    }
    visitState[id] = 2;
  };
  for (const n of nodes) if (!visitState[n.id]) walk(n.id);
  const layerEdges = forward.filter(e => !backEdges.has(e));
  ...
  const ordered = layers.filter(Boolean);
  reduceCrossings(ordered, edges);
  layoutLayers(ordered);
}

// Barycenter ordering: repeatedly place each node next to the average
// position of its neighbours in the adjacent layer. A few alternating
// sweeps turn a spaghetti of long diagonals into readable columns.
function reduceCrossings(layers, edges) {
  ...
  const pos = {};
  const reindex = () => layers.forEach(l => l.forEach((n, i) => { pos[n.id] = i; }));
  reindex();

  const sweep = (indices) => {
    for (const li of indices) {
      const fixed = new Set([...(layers[li - 1] || []), ...(layers[li + 1] || [])].map(n => n.id));
      const bary = new Map(layers[li].map(n => {
        const seen = (neighbours[n.id] || []).filter(id => fixed.has(id)).map(id => pos[id]);
        return [n.id, seen.length ? seen.reduce((a, b) => a + b, 0) / seen.length : pos[n.id]];
      }));
      layers[li] = layers[li]
        .map((n, i) => ({ n, i }))
        .sort((a, b) => (bary.get(a.n.id) - bary.get(b.n.id)) || (a.i - b.i))
        .map(x => x.n);
      reindex();
    }
  };

  const down = layers.map((_, i) => i);
  const up = [...down].reverse();
  for (let pass = 0; pass < 4; pass++) sweep(pass % 2 ? up : down);
}
...
// ---------------------------------------------------------------------------
// Mermaid / arrow-syntax import
//
// Structured topology definitions are parsed EXACTLY (no guessing):
//
//   flowchart TD
//   subgraph DMZ
//     FC01[Firewall 1] -->|"Bond1 - External"| DMZSW
//     FC02 -.-> DMZSW
//   end
//   DMZSW --> RB01 & RB02
//
// Supported: graph/flowchart headers, node shapes (A[..] A(..) A((..))
// A[(..)] A{..} A{{..}} A[[..]]), edge labels (-->|label| and
// -- label -->), dotted (-.->) and thick (==>) links, chains
// (A --> B --> C), multi-node groups (A --> B & C), subgraph blocks
// (which become zones), and %% comments. Everything else (classDef,
// style, click, linkStyle) is ignored rather than treated as a device.
// ---------------------------------------------------------------------------

const MERMAID_MAX_NODES = 200;
const MERMAID_MAX_EDGES = 400;

// Dotted first, then thick, then plain; each may carry a |label|.
const MERMAID_ARROW =
  /(-\.+->|-\.+-|={2,}>|={3,}|-{2,}>|-{3,}|--[ox])(?:\s*\|\s*(?:"([^"]*)"|'([^']*)'|([^|]*?))\s*\|)?/g;

function looksLikeMermaid(text) {
  if (/^\s*(?:graph|flowchart)\s+[A-Za-z]{2}\b/im.test(text)) return true;
  const lines = text.split('\n').map(l => l.trim()).filter(Boolean);
  let arrows = 0;
  for (const line of lines) {
    MERMAID_ARROW.lastIndex = 0;
    if (MERMAID_ARROW.test(line)) arrows++;
  }
  return arrows >= 2 || (arrows === 1 && /^\s*subgraph\b/im.test(text));
}

// "NGA-SC01<br/>10.0.0.1" → {label: 'NGA-SC01', sub: '10.0.0.1'}
function mermaidLabel(raw) {
  ...
  return { label: (first || '').slice(0, 40), sub: rest.join(' ').slice(0, 40) };
}

// "FC01[Firewall 1]" → {id, label, shape}
function mermaidNodeToken(token) {
  ...
  if (m[2]) {
    shape = m[2].slice(0, 2);
    label = m[2].replace(/^[[({>]+/, '').replace(/[\])}/\\]+$/, '');
  }
  return { id: m[1], label, shape };
}

// Mermaid shapes that imply a device kind; otherwise the name decides.
const MERMAID_SHAPE_TYPES = { '[(': 'database', '((': 'cloud', '[[': 'server' };

// Type from a node's id and label: device words in the label win, then
// hostname hints, then a generic server.
function inferTypeFromName(id, label) {
  const text = `${label || ''} ${id || ''}`;
  for (const { type, re } of DEVICE_VOCAB) {
    re.lastIndex = 0;
    if (re.test(text)) return type;
  }
  return inferTypeFromHostname(id || label || '');
}
...
function parseMermaid(text) {
  ...
  const ensure = (id, label, shape) => {
    let n = nodes.get(id);
    if (!n) {
      n = { id, label: id, sub: '', shape: shape || '' };
      nodes.set(id, n);
    }
    if (label) {
      const parsed = mermaidLabel(label);
      if (parsed.label) n.label = parsed.label;
      if (parsed.sub) n.sub = parsed.sub;
      n.named = true;
    }
    if (shape && !n.shape) n.shape = shape;
    if (stack.length) {
      const zone = stack[stack.length - 1];
      if (!zone.ids.includes(id)) zone.ids.push(id);
    }
    return n;
  };

  for (const rawLine of String(text).split('\n')) {
    let line = rawLine.replace(/%%.*$/, '').trim().replace(/;+$/, '');
    if (!line) continue;
    if (/^(?:graph|flowchart)\b/i.test(line)) continue;
    if (/^(?:classDef|class|style|linkStyle|click|direction|title|accTitle|accDescr)\b/i.test(line)) continue;

    const sg = /^subgraph\s+(.*)$/i.exec(line);
    if (sg) {
      const token = mermaidNodeToken(sg[1]);
      const label = token && token.label ? mermaidLabel(token.label).label
        : mermaidLabel(sg[1]).label || 'Zone';
      const zone = { label, ids: [] };
      zones.push(zone);
      stack.push(zone);
      continue;
    }
    ...
    // "A -- text --> B" is the same as "A -->|text| B"; normalize so one
    // arrow pattern handles both.
    line = line
      .replace(/--\s+([^->|]+?)\s+-->/g, '-->|$1|')
      .replace(/==\s+([^=>|]+?)\s+==>/g, '==>|$1|');
    ...
    const links = [];
    let last = 0, m;
    while ((m = MERMAID_ARROW.exec(line)) !== null) {
      parts.push(line.slice(last, m.index));
      last = MERMAID_ARROW.lastIndex;
      links.push({
        label: (m[2] || m[3] || m[4] || '').trim(),
        style: /^-\.+/.test(m[1]) ? 'dashed' : 'solid'
      });
    }

    if (!links.length) {
      // A line with no arrow is a node declaration ("A[Label]") or a bare
      // reference — inside a subgraph the bare form is how membership is
      // declared, so register it either way.
      for (const part of line.split('&')) {
        const token = mermaidNodeToken(part);
        if (token && /^[A-Za-z0-9_.:-]+$/.test(token.id)) {
          ensure(token.id, token.label, token.shape);
        }
      }
      continue;
    }
    ...
    // "A --> B & C" fans out to every node in the group.
    const groups = parts.map(part =>
      part.split('&').map(mermaidNodeToken).filter(Boolean));
    for (const group of groups) {
      for (const token of group) ensure(token.id, token.label, token.shape);
    }
    for (let i = 0; i + 1 < groups.length; i++) {
      for (const a of groups[i]) {
        for (const b of groups[i + 1]) {
          edges.push({ from: a.id, to: b.id, label: links[i].label, style: links[i].style });
        }
      }
    }
  }
  ...
  const list = [...nodes.values()].map(n => ({
    ...n,
    type: MERMAID_SHAPE_TYPES[n.shape] || inferTypeFromName(n.id, n.named ? n.label : '')
  }));
  return { nodes: list, edges, zones: zones.filter(z => z.ids.length) };
}

// Build the diagram from Mermaid text. Returns true when it produced one.
function generateFromMermaid(text) {
  const parsed = parseMermaid(text);
  if (!parsed.nodes.length) return false;

  const idMap = {};
  const nodes = parsed.nodes.slice(0, MERMAID_MAX_NODES).map(n => {
    const icon = NETWORK_ICONS[n.type] || NETWORK_ICONS.server;
    const node = {
      id: uid('n'), type: n.type, x: 0, y: 0,
      label: n.label, sub: n.sub || '', color: icon.color
    };
    idMap[n.id] = node.id;
    return node;
  });
  ...
  for (const e of parsed.edges) {
    ...
    if (seen.has(key)) {
      // Both directions of the same link are one line; keep any label.
      const existing = seen.get(key);
      if (e.label && !existing.label) existing.label = e.label.slice(0, 40);
      continue;
    }
    if (edges.length >= MERMAID_MAX_EDGES) break;
    const edge = {
      id: uid('e'), from, to,
  ...
  const zoneGroups = parsed.zones.map(z => ({
    label: z.label,
    nodes: z.ids.map(id => nodes.find(n => n.id === idMap[id])).filter(Boolean)
  })).filter(z => z.nodes.length);

  applyGenerated(nodes, edges, text,
    `Mermaid syntax: ${nodes.length} devices, ${edges.length} links` +
    (zoneGroups.length ? `, ${zoneGroups.length} zone(s)` : ''),
    makeZones(zoneGroups));
  return true;
}
...
function handleGenerate() {
  ...
  // Structured arrow syntax is parsed exactly — never guessed at, and
  // never sent to an AI backend.
  if (looksLikeMermaid(text) && generateFromMermaid(text)) return;
